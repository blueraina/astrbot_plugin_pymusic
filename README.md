...
v0.4.4 的重点是把 `musicpy` 改为强依赖，并用于本地作曲层：插件会基于调式/和弦生成多条旋律候选，按强拍和弦音、句尾解决、大跳后反向解决、音域、重复度等规则评分，选出更耐听的 call motif，再派生 response 和 B variation。AI Python renderer 沙箱还提供安全数组 helper，减少 sidechain、delay、reverb、fill 等局部数组写入越界的问题。最终 WAV 仍由受限 AI Python renderer 或固定 NumPy renderer 输出。
...
v0.4.4 的代码生成提示会明确要求 AI 使用 `composition_blueprint` / `structured_composer_plan`，并实现：
...
- v0.4.4 使用的是 `musicpy` 辅助作曲、本地结构化作曲计划、变化种子、AI 代码修复、安全数组 helper 和更强的兜底渲染，不会让 AI 获得任意 Python 执行能力。
