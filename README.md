...
`astrbot_plugin_pymusic` 是一个 AstrBot 插件，可以根据用户提示词，用 `musicpy` 辅助作曲，并用纯 Python / NumPy 合成 WAV 音乐，然后发送到 QQ 群或私聊。插件不调用外部音乐生成 API，不使用采样包；模型仍然只允许输出受限的 `render(duration, sample_rate, loopable)` Python 函数。

v0.4.6 的重点是把 `musicpy` 改为强依赖，并用于本地作曲层：插件会基于调式/和弦生成多条旋律候选，按强拍和弦音、句尾解决、大跳后反向解决、音域、重复度等规则评分，选出更耐听的 call motif，再派生 response 和 B variation。AI Python renderer 沙箱还提供安全数组 helper，减少 sidechain、delay、reverb、fill 等局部数组写入越界的问题，放宽 `globals` / `locals` / `vars` / `dir` / `help` 为安全自检函数，并允许 AI 代码 `import musicpy as mp` 用于作曲辅助。最终 WAV 仍由受限 AI Python renderer 或固定 NumPy renderer 输出。

## 功能

- 指令：`/pymusic 时间(秒) 提示词`
- Agent 工具：`generate_python_music`
- `musicpy` 作为可选的作曲/音乐理论辅助层（未安装时自动回退到纯 Python 作曲）
- 使用 `numpy` 和 Python 标准库渲染 WAV，输出宽立体声（mono 兼容）
- 不接入外部音乐生成 API
- 保留 AI 直接写受限 Python `render()` 的主路径
- AI 生成代码继续经过 AST 校验和子进程沙箱执行
- AI 代码生成使用独立的更长超时（默认 120 秒），并最多自动修复 2 次，显著提高 AI 主路径成功率
- 模型失败、代码校验失败、执行失败或未生成有效 WAV 时，自动回退到固定 `PythonMusicRenderer`
- 合成层使用 unison 失谐叠音、谐振低通滤波、sub 低频强化和逐层饱和，让音色更厚更暖
- 旋律使用随机节奏栅格、休止/切分、放宽的跳进评分和多套和弦进行，同提示词多次生成会有不同旋律
- 仅支持 QQ 个人号适配器和 QQ 官方机器人适配器
- 支持 `voice`、`file`、`auto` 三种发送模式
- 语音模式最长 60 秒，超过 60 秒自动改为文件发送
- 插件硬上限默认 600 秒，可在 WebUI 修改
- WebUI 支持模型选择、模型调用超时、AI 代码生成超时、最大时长、默认时长、多样性强度、确定性模式、每次生成变化强度、波形级无缝循环、默认发送模式、采样率、是否保留历史 WAV
...
-> musicpy-assisted melody candidates / melody scoring
...
- 使用 `musicpy` 建立调式上下文，并本地生成多条 call motif 候选。
- 通过旋律评分器选择更耐听的候选：强拍和弦音、句尾解决、大跳后反向解决、音域控制、重复度控制。
...
- 允许导入 `numpy`、`math`、`random`、`musicpy`。
- 禁止读写文件、访问网络、调用系统命令。
- 禁止 `os`、`sys`、`subprocess`、`pathlib`、`open`、`eval`、`exec`、`__import__` 等危险能力。
- 禁止外部采样和外部音乐生成 API；`musicpy` 只允许作为本地作曲辅助，不允许调用文件导出、播放或 DAW 相关出口。
- `globals`、`locals`、`vars`、`dir`、`help` 会被替换成安全版本，只能看到白名单对象。
- 生成代码会先做 AST 校验，再写入临时代码文件，由独立 Python 子进程加载并渲染 WAV。

v0.4.6 的代码生成提示会明确要求 AI 使用 `composition_blueprint` / `structured_composer_plan`，并实现：
...
- 使用沙箱内置的 `safe_add`、`safe_assign`、`safe_min_assign`、`safe_multiply` 做局部数组写入，避免结尾鼓点、sidechain 包络或 delay tail 越界
...
`musicpy` 使用 LGPLv2.1 许可证。本插件只把它作为 pip 依赖调用，用于音阶、和弦与旋律候选生成；不会复制或修改 `musicpy` 源码，也不会使用其 DAW / SoundFont / VST 音频导出能力。
...
- 插件不会调用外部音乐生成服务。
- AI 代码沙箱开放 `numpy`、`math`、`random`、`musicpy`，但继续禁止文件、系统和网络出口。
- v0.4.6 使用的是 `musicpy` 辅助作曲、本地结构化作曲计划、变化种子、AI 代码修复、安全数组 helper 和更强的兜底渲染；AI renderer 可导入 `musicpy` 做本地作曲辅助，但不会获得任意 Python 执行能力。
